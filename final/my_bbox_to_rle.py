...
def read_label_csv():
    ...
    for i in range(len(data)):
        ...
        s = str(parser[0])
        s = s.zfill(4)
        filepath = "test" + str(s) + ".png"
        if parser[5] == 1:
            dic = {"id": filepath, "x": float(parser[1]), "y": float(parser[2]), "w": float(parser[3]), "h": float(parser[4]),
                       "img_w": img_w, "img_h": img_h}
            bboxs.append(dic)
        elif parser[5] == 0:
            img_w, img_h = (1024, 1024)
            dic = {"id": filepath, "x": 0.0, "y": 0.0, "w": 0.0, "h": 0.0, "img_w": img_w, "img_h": img_h}
            bboxs.append(dic)
