...
def train(opt):
    ...
    training_set = VOCDataset('data/norm_train_image/', 'data/train_labels.csv',data_augmentation = True)
    training_generator = DataLoader(training_set, **training_params)

    test_set = VOCDataset('data/norm_train_image/', 'data/val_labels.csv',data_augmentation = False)
    ...
    for epoch in range(opt.num_epoches):
        ...
        for iter, batch in enumerate(training_generator):
            ...
            progress = (u"\u2588" * (int(float(iter+1)/num_iter_per_epoch*40))
                        ).ljust(40, '.')
            msg = '[%03d/%03d] %2.2f sec(s) |%s| Current Loss:%.2f' % (iter+1, num_iter_per_epoch,
        ...
        if epoch % opt.test_interval == 0:
            ...
            print("\nEpoch: {}/{}, Lr: {}, Train Loss:{:.2f} Val Loss:{:.2f} (Coord:{:.2f} Conf:{:.2f} Cls:{:.2f})".format(
                epoch + 1,
                opt.num_epoches,
                optimizer.param_groups[0]['lr'],
                train_loss/opt.num_epoches,
                te_loss,
                te_coord_loss,
                te_conf_loss,
                te_cls_loss))
