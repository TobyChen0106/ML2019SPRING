...
class FocalLoss(nn.Module):
    def __init__(self, alpha=0.25, gamma=2.0, loss_with_no_bboxes=False, no_bboxes_alpha=0.5, no_bboxes_gamma=2):
        self.alpha = alpha
        self.gamma = gamma
        self.loss_with_no_bboxes = loss_with_no_bboxes
        self.no_bboxes_alpha = no_bboxes_alpha
        self.no_bboxes_gamma = no_bboxes_gamma
    ...
    def forward(self, classifications, regressions, anchors, annotations):
        ...
        for j in range(batch_size):
            ...
            if bbox_annotation.shape[0] == 0 and self.loss_with_no_bboxes == False:
            ...
            #This is the part of the code where those pictures without bboxes are take into account(customized by Yu Han Huang)
                ...
                targets = 1 - targets
                alpha_factor = torch.ones(targets.shape).cuda() * self.no_bboxes_alpha
                alpha_factor = torch.where(torch.eq(targets, 1.), alpha_factor, 1. - alpha_factor)
                focal_weight = torch.where(torch.eq(targets, 1.), 1. - classification, classification)
                focal_weight = alpha_factor * torch.pow(focal_weight, self.no_bboxes_gamma)
                bce = -((1.0 - targets) * torch.log(1.0 - classification))
                cls_loss = focal_weight * bce
                classification_losses.append(cls_loss.sum()/torch.clamp(targets_num.float(), min=1.0))
